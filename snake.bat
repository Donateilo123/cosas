...
for /l %%. in (1 0 1) do (
  ...
  if !tDiff! geq !delay! (
    ...
    (%test% !X! !Y! playerSpace) && (
      ...
      %draw%
      echo(
      call :ask "Te gustaría volver a jugar? (Y/N)" YN
      if /i "!key!" equ "N" (
        >"%cmdFile%" (echo quit)
        exit
      ) else (
...
echo Speed Options:
echo                       fps
echo    #   Descripcion  (seconds)
echo   ---  -----------  ---------
echo    1   Perezoso       0.20
echo    2   Gatear         0.15
echo    3   Lento          0.10
echo    4   Normal         0.07
echo    5   Rápido         0.05
echo    6   Loco           0.03
echo    0   Injugable      none
echo(
call :ask "Elige una velocidad (1-6, 0):" 1234560
set "difficulty=!desc%key%!"
set "delay=!delay%key%!"
echo %key% - %difficulty%
echo(
<nul set /p "=Inicializar."
set "axis=X"
set "xDiff=+1"
set "yDiff=+0"
set "empty="
...
(%draw%)
echo(
echo Teclas de movimiento: %up%=up %down%=down %left%=left %right%=right
echo Evita toparte contigo mismo (%body%%body%%head%) or wall (%bound%)
echo Eat food (%food%) to grow.
echo(
call :ask "Presione cualquier tecla alfanumérica para comenzar..." %keys%
>"%cmdFile%" (echo go)
exit /b
